app.controller("take", function ($scope, ajax, $window, $http, $routeParams) {
    ...
    if (localStorage.getItem('token') != null) {
        $http.get("https://localhost:44366/api/packages/")
            .then(resp => $scope.package = resp.data);

        $http.get("https://localhost:44366/api/hotels/")
            .then(resp => $scope.hotel = resp.data);

        $scope.submit = () => {
            console.log($scope.formData);
            ajax.post("https://localhost:44366/api/product/add", $scope.formData);
            ajax.get("https://localhost:44366/api/pdf", success, error);
        }
    ...
    $('#package').change(function () {
        if (this.value != undefined) {
            ajax.get("https://localhost:44366/api/allproduct" + this.value, success, error);
        }
